...
int main()
{
    int z, i, j, k, l; // ciklu mainigie
    int m; // Kopas izmeers ko ievadiis lietotajs
    ...
    for (z=0; z<MasivaIzmers; z++) // izvada SAKAARTOTA masiiva burtu veidaa un atbistosu ASCII koda veidaa
        if (mas3[z] == 0)
            printf("Simbols: 'atstarpe'\tASCII kods: %d\n", mas3[z]);
        else
            printf("Simbols: %c\t\tASCII kods: %d\n", mas3[z], mas3[z]);
    ...
    printf("(b) Minimaala elementa veertiiba: Pagaidam nav aprekinats \n");
    printf("(c) Maksimaala elementa veertiiba: Pagaidam nav aprekinats \n");
